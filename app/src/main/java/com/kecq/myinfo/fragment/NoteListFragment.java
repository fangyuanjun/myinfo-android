...
public class NoteListFragment extends ListTitleFragment {
    ...
    protected String getQueryDataUrl() {
        return Temp.MyInfoBaseUrl + SignHelper.SignUserUrl("/Note/GetList");
    }
    ...
    protected List<ListModel> resultToListModel(String result) throws JSONException {
        ...
        for (int i = 0; i < arry.length(); i++) {
            ...
            model.setTitle(itemobj.optString("noteContent"));
            model.setGuid(itemobj.optString("noteID"));
    ...
    protected String getDeleteUrl(String id) {
        return Temp.MyInfoBaseUrl+SignHelper.SignUserUrl("/Note/Remove");
    }
    ...
    protected void jumpDetail(ListModel model) {
        ...
        intent.putExtra("title","修改随记");
        intent.putExtra("url",Temp.MyInfoBaseUrl+ SignHelper.SignUserUrl("/Note/PhoneEdit/"+id) );
