...
OnClickListener{
	...
	private IRefreshLoadingMoreListener onRefreshLoadingMoreListener;// 下拉刷新接口（自定义）
	...
	public void setOnRefreshListener(
			IRefreshLoadingMoreListener onRefreshLoadingMoreListener) {
	...
	public void initDragListView(Context context) {
        ...
        {
        	time=this.onRefreshLoadingMoreListener.getInitRefreshDatetime();
        }
	 ...
	 * 更新最后刷新时间
	 */
	public void setRefreshDatetime(String datetime)
	{
		mLastUpdateTextView.setText("最近更新:" + datetime);
	}
	/***
	 * 点击加载更多
	 * 
	...
	/***
	 * ListView 滑动监听  onScrollStateChanged暂时没用到 
	 */

	public void onScrollStateChanged(AbsListView view, int scrollState) {
		// TODO Auto-generated method stub
		//一般执行顺序为：SCROLL_STATE_TOUCH_SCROLL ---》SCROLL_STATE_FLING---》SCROLL_STATE_IDLE
		switch (scrollState) {
		...
		case OnScrollListener.SCROLL_STATE_IDLE: //滑动后静止   Idle态，进行实际数据的加载显示
			//isScrolling = false;
			  // 判断滚动到底部
			...
			for (int i = 0; i < count; i++) {
				...
				final Article model = (Article) lv_main_actices
						.getItemAtPosition(currentpostion);
				final View viewchildren = lv_main_actices.getChildAt(i);
				SetItemView(viewchildren, model);
